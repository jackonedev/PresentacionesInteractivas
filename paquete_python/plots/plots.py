# este fichero proviene de jackone/idea6/timeseries-project/paquete_proyecto/data_report/plots.py
...
class VisualProcessor(ABC, VisualizerData):
    ...
    - su objetivo final es obj.set_content().plot(), .plot_area, .plot_kind(kind=)
    ...
    def set_content(
        ...
        # Or Collect DataFrame from list by iterator
            ...
            if self.current_content is None:
                raise StopIteration("No hay mas DataFrames para procesar")
        # Ordering columns#TODO: en caso que sean todas numéricas
        try:
        ...
        if y_label is not None:
            ...
            #TODO 2: estoy probando algo
            if isinstance(self.y_axis_label, dict):
                self.y_axis_label = self.y_axis_label["value"]
